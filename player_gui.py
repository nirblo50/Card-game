...
def what_was_clicked(game_status, mouse_pos):
    if game_status == "Game did not start yet":
        return
    ...
    if button_x <= mouse_x <= button_x + card_width and button_y <= mouse_y <= button_y + card_height:
        return "Button", -1
    return None, None


def click_to_action(_what_was_clicked):
...
if __name__ == '__main__':
    ...
    action = ["Get"]  # What to send the server
